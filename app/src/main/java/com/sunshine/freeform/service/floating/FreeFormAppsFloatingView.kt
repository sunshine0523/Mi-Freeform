...
class FreeFormAppsFloatingView(
    ...
    fun showFloatingViewWindow() {
        if (Settings.canDrawOverlays(context)) {
            ...
            floatingViewLayoutParams.apply {
                ...
                var width = 0
                //横屏
                width = if (FreeFormConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
                    max(point.x, point.y)
                } else {
                    //竖屏
                    min(point.x, point.y)
    ...
    private fun setFloatingViewContent(floatingView: View) {
        ...
        recyclerView.adapter = FreeFormAppsFloatingAdapter(context,
            floatingApps, sp?.getInt("freeform_display_model", 1)?:1, object : com.sunshine.freeform.callback.FloatingClickListener {
