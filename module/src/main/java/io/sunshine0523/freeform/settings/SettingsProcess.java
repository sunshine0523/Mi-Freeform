...
import io.sunshine0523.freeform.shortcut.SuiShortcut;
...
public class SettingsProcess {
    ...
    private static void postBindApplication(ActivityThread activityThread) {
        MiFreeformApk miFreeformApk = MiFreeformApk.createForSettings();
        if (miFreeformApk == null) {
            LOGGER.e("Cannot load apk");
            return;
        }
        LOGGER.e("Mi-FreeformAPK" + miFreeformApk);

        Instrumentation instrumentation = ActivityThreadUtil.getInstrumentation(activityThread);
        SettingsInstrumentation newInstrumentation = new SettingsInstrumentation(instrumentation, miFreeformApk);
        ...
        if (resources != null) {
            handlerThread = new HandlerThread("MF");
            handlerThread.start();
